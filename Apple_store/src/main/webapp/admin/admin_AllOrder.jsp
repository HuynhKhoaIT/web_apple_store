				...
				<tbody>
					<c:forEach var="i" begin="0" end="${size-1 }">
						<tr>
							...
							<td><a href="Ad_OrderDetail?id=${list[i].getMaDH()}"
								class="btn btn-sm btn-secondary ">Chi tiết</a></td>
							<td><a
								href="ChangeStatus?idDonHang=${list[i].getMaDH()}&status=${list[i].getMaTrangThai()}"
								class="btn btn-sm btn-secondary ">Change</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
			<a href="<%=request.getContextPath()%>/admin"
				class="btn btn-sm btn-secondary ">Exit</a>
		</div>
	</div>
</body>
</html>